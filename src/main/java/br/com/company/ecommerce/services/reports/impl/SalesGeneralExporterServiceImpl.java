...
public class SalesGeneralExporterServiceImpl implements ReportExporterService {

    private static final String[] HEADER = { "month", "count", "sum" };
    ...
    public boolean supports(ReportType type) {
        return ReportType.SALES_GENERAL.equals(type);
    }

    @Override
    public void export(Report report) throws IOException {
        Pageable pageable = PageRequest.of(0, 100);
        Page<SaleGeneralReport> page = null;
        ...
        try (CSVWriter writer = new CSVWriter(path, report.getFilename())) {
            ...
            do {
                page = repository.findAllSales(accountId, startAt, endAt, pageable);

                for (SaleGeneralReport item : page.getContent()) {
                    String[] fields = {
                            item.getMonth(),
